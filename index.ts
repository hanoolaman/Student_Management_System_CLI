...
async function mainMenu() {
    ...
    if(menu === "3. View Student Balance")await viewStudentBalance();
    if(menu === "4. Pay Course Fee")await payTuition();
...
async function selectStudent() {
    if(students.length === 0){
        ...
        return(
            students.find((std) => std.StudentID === stdSelect.stdID) || null)
    }
}
mainMenu();